...
leg1_w = 0.01;
leg1_h = 0.01;
dim(:,link) = [leg1_l; leg1_w; leg1_h];
joint_to_com(:,link) = [0; leg1_l/2; 0]; % p_ (position vector pointing from j2 to COM)
rot(3,link) = -pi/2;
link = link+1;

% 5) invisiblelink(invl) 3 (leg1 to j3)
dim(:,link) = [invl_l; invl_w; invl_h]; %dim(:,2);%[invl_w; invl_l; invl_h];
joint_to_com(:,link) = [-leg1_l/2; 0; 0];
rot(3,link) = pi/2;
link = link+1;
...
leg2_w = 0.01;
leg2_h = 0.01;
dim(:,link) = [leg2_l; leg2_w; leg2_h];
joint_to_com(:,link) = [0; leg2_l/2; 0];
rot(3,link) = -pi/2;
link = link+1;
