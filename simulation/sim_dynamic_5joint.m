...
filename = "xtraj_KE_5dof_dampgrav.mat"; % "xtraj_KE_5dof.mat" "xtraj_KE_5dof_damp.mat" "xtraj_KE_5dof_dampgrav.mat"
if ~isfile(filename)
    x_traj = zeros(size(x0,1),n);
    KE = zeros(n, 1);
    x_traj(:,1) = x0;
    KE(1) = ke_test_5dof(x_traj(:,1));
    
    for i=2:n
        bolddotx_t = solve_5dof(x_traj(:,i-1), damp_mat);
        x_traj(:,i) = x_traj(:,i-1) + bolddotx_t * dt; % forward euler
        KE(i) = ke_test_5dof(x_traj(:,i));
    end
    
    save(filename, "x_traj", "KE");
else
    load(filename);
end
...
if(show_plot)
    ...
    for ts=1:n_speedup
        ...
        % display now config
        q_aug = [0; 0; x_traj(1,t); x_traj(2,t); 0; x_traj(3,t); 0; x_traj(4,t); x_traj(5,t); q6];
        T = solve_kinematics(q_aug, joint_to_com, rot);
        handle = plot_single_legged_robot(T, dim, 0);
        drawnow limitrate;
    end
