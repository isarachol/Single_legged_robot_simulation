function [bolddotx] = f_twolinkrobot_springdampgrav(x, constants)
%f_doublepend The dynamics function f(x), in state-space form, for a
%system of N=2 rigid bodies as a double pendulum now with gravity
%
...
% Gravitational forces (generalized)
G = [m1*grav*ell1*cos(q1) + m2*grav*(ell2*cos(q1+q2) + a1*cos(q1));
     m2*grav*ell2*cos(q1+q2)];
